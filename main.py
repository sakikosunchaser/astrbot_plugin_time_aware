...
class TimeAwarePlugin(Star):
    ...
    def load_config(self):
        ...
        try:
            ...
            print(f"[{self.plugin_name}] load config failed: {e}")
            return dict(self.default_config)
    ...
    def get_now_in_beijing(self):
        if ZoneInfo is None:
            return datetime.now()
        return datetime.now(ZoneInfo(self.config.get("timezone", "Asia/Shanghai")))
    ...
    def render_debug_summary(self, user_message):
        now = self.get_now_in_beijing()
        period = self.get_time_period(now)
        period_zh = self.get_time_period_zh(period)
        style_hint = self.get_style_hint(period)
